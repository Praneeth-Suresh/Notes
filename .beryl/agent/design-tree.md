...
| Homepage staged animation reference | Use `.beryl/agent/animations.md` as the durable choreography reference for homepage scroll motion: title first, action lane second, sticky hold third, stable clickable targets, and preserved palette transitions. | 2026-06-24 | n/a |
...
| About page linear reveal | Remodel `/about/` as a Kyle Chung-inspired linear scroll: minimal hero, CSS-only signal-weave animation, concise sections that progressively disclose identity, method, proof links, selected work, opportunities, and repository terrain. | 2026-07-06 | n/a |
...
- Homepage showcase motion now follows `.beryl/agent/animations.md`: staged left-to-right reveals, sticky desktop holds, stable action/card targets after reveal, static mobile/reduced-motion fallback, and the existing palette transitions.
...
- The repository control plane now uses Beryl's `.beryl/` layout with `.beryl/agent`, `.beryl/scripts`, `.beryl/driver`, and `.beryl/githooks`; generated root shims point future agents at `.beryl/agent/`.
