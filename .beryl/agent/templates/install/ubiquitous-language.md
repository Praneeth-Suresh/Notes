...
| Affected Test Gate | `.beryl/scripts/check-affected.sh` | Commit and manual check step that maps changed files to related tests or full-test fallback. | Must be deterministic and run through `.beryl/.beryl/scripts/check.sh`. | Ad hoc local test picking |
