...
If the file being changed lives under a directory that contains its own `.beryl/agent/` folder, that module's agent files take precedence for module-specific concerns.

### Loading Order

1. Read the module's `.beryl/agent/module-context.md` to understand scope and precedence.
2. Load the module's `.beryl/agent/project-brief.md`, `.beryl/agent/design-tree.md`, `.beryl/agent/architecture.md`, `.beryl/agent/ubiquitous-language.md`, and `.beryl/agent/testing-policy.md`.
...
2. Load each module's `.beryl/agent/module-context.md`.
...
Run the module's own check command (from its `.beryl/agent/testing-policy.md`) for narrow checks, then the root `./.beryl/scripts/check.sh` for the full gate.
