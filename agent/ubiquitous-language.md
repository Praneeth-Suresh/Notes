...
| Blog Manifest | `BlogManifest` | Registry of blog sections, posts, and their source markdown files, generated by the wiki pull script. | Must be checked in and consumed at build time. | `PostList` |
| Blog Post | `BlogPost` | A single blog entry derived from a GitHub wiki page. | Has a slug, title, optional chapter number, and markdown source. | `Article` |
| Blog Section | `BlogSection` | A thematic grouping of blog posts (e.g. "A Developer's Memoir", "Readings in AI"). | Defined in the wiki sidebar and preserved in the manifest. | `Category` |